	...
	const renderPageNumbers = () => {
		...
		pageNumbers.push(
			<button
				...
				className={`pagination_button ${currentPage === 1 ? 'pagination_button_disable' : ''}`}
			>
				{`<`}
			</button>
		);
		...
		if (totalPages > 1) {
			pageNumbers.push(
				<button
					...
					className={`pagination_button ${currentPage === totalPages ? 'active' : ''}`}
				>
					{totalPages}
				</button>
			);
		...
		pageNumbers.push(
			<button
				...
				className={`pagination_button ${currentPage === 10 ? 'pagination_button_disable' : ''}`}
			>
				{`>`}
			</button>
		);
				...
				<div className='main'>
					...
					<div className='table'>
						<table>
							<thead>
								<tr>
									<th className='w-[445px] text-start px-5 text-[14px] text-black'>Coin</th>
									<th className='w-[240px] text-right text-[14px] text-black'>Price</th>
									<th className='w-[258px] text-right text-[14px] text-black'>24h Change</th>
									<th className='w-[250px] text-right text-[14px] text-black'>Market Cap</th>
								</tr>
							</thead>
							<tbody>
								{currentItems.length > 0 &&
									currentItems.map(crypto => {
										const isAdded = watchList.some(item => item.id === crypto.id);
										return (
											<tr
												key={crypto.id}
												...
												<td className='w-[240px] text-right'>
													<p className='text-[14px] text-white'>
														{current === '₹'
															...
															: current === '$'
															? `${current} ${convertCurrency(crypto.current_price, 0.012)} `
															: ` ${current} ${convertCurrency(crypto.current_price, 0.0111)}`}
													</p>
												</td>
													...
													<div className='flex justify-end items-center '>
														<div className='flex items-center mt-[10px]'>
															<div
																...
																onClick={() =>
																	isAdded ? removeFromWatchList(crypto.id) : addToWatchList(crypto)
																}
															>
																<span
																	className={`${
